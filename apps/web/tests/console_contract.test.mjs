...
test('P0 console contains real browser operations for the control-plane flow', () => {
  ...
  for (const snippet of [
    ...
    '/deployments/${deployment.deploymentId}/readiness',
    '/deployments/${deployment.deploymentId}/snapshot',
    '/deployments/${deployment.deploymentId}/rollback-pointer',
    '/artifacts/${deployment.artifactId}/bytes',
    '/subscriptions/${profileId}',
    '/subscriptions/${profileId}/tokens',
    '/subscriptions/${profileId}/tokens/rotate',
    '/runner/nodes/${nodeId}/usage',
    '/usage/nodes/${nodeId}/latest',
    '/runner/nodes/${nodeId}/heartbeat',
    '/nodes/${nodeId}/heartbeat',
    '/runner/nodes/${nodeId}/commands/next',
    '/runner/nodes/${nodeId}/results',
    ...
    '/usage/credentials/${clientId}/rollups/latest?bucket=day',
    '/usage/credentials/${clientId}/rollups/latest?bucket=month',
    '/system/capabilities',
...
test('dashboard uses a Vercel-style workbench with a Claude-style operator artifact rail', () => {
  ...
  for (const snippet of [
    ...
    'Rotate key',
    'Runner result public key',
    'sent as x-runner-token',
    'Node inventory',
    'A node is a VPS-side runner identity',
    'Runner queue and browser journal',
    'Runner result sink',
    'runner binary submits signed apply results',
    'Quota evidence',
    'Architecture status',
    'Backend wheels',
    'Capability matrix',
